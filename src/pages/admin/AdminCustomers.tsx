  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {filtered.map((c) => (
          <div key={c.id} className="rounded-2xl border border-border bg-card p-5 shadow-soft transition-all hover:shadow-card hover:-translate-y-0.5">
            ...
            <div className="mt-4 flex items-center justify-between border-t border-border pt-3">
              ...
              <div className="text-end">
                <p className="text-xs text-muted-foreground">{t('admin.lastOrder')}</p>
      ...
      {filtered.length === 0 && (
        <p className="py-8 text-center text-sm text-muted-foreground">{t('admin.noCustomers')}</p>
      )}
    </div>
  );
};
